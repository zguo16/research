function [ z ] = TomoRandRecovPmfandCoeff(coeff, basis, pmf_theta,num_of_delta_alpha,delta_theta,n_r)
...
z0 = [randn(size(a1)) + sqrt(-1)*randn(size(a1));randn(2*k_max,1)+1i*randn(2*k_max,1)]; %Because of symmetry, we just need ot initialize the positive coefficients. random initialization, need to think about a better way to initialize
F = @(z)objfun(z, C, Psi, p0,size_of_a,k_max,coeff);
...
function [ cost ] = objfun( z, C, Psi, p0, size_of_a, k_max,coeff )

z1_minus = conj(flipud(z(p0+1:size_of_a))); %negative angular frequency coefficients
z1 = [ z1_minus; z(1:size_of_a) ]; %add the negative angular frequency coefficients.
z2 = [1;z(size_of_a+1:end)];
temp4 = 0;
 
 for i = -k_max:1:k_max    
     temp5 = 0;
     for j = -k_max:1:k_max
         if abs(i-j)<=size(z2,1)-1
             if i-j<0
                 C_P(temp4+1:temp4+size(coeff{abs(i)+1,1},1),temp5+1:temp5+size(coeff{abs(j)+1,1},1)) = ...
                   z2(abs(i-j)+1,1);
             else
